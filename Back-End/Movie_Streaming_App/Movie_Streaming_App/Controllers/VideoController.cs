...
namespace Movie_Streaming_App.Controllers
{
    ...
    public class VideoController : ControllerBase
    {
        ...
        [HttpGet("watch")]
        public async Task<IActionResult> WatchVideo(string name)
        {
            using var client = new HttpClient();

            #region Finding Youtube Video ID

            var response = await client.GetAsync($"https://www.googleapis.com/youtube/v3/search?part=snippet&q={name + "Official Trailer"}&type=video&key={_configuration["YoutubeWatchKey"]}");
            if (!response.IsSuccessStatusCode)
                return NotFound();
            string? videoId;
            try
            ...
            var request = new HttpRequestMessage
            {
                ...
                Headers =
                {
                    { "x-rapidapi-key", _configuration["RapidApiWatchKey"] },
                    { "x-rapidapi-host", _configuration["RapidApiHost"] },
                },
            };
            response = await client.SendAsync(request);
